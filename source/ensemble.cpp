...
int Ensemble::predict(trace* trace) const {
    ...
    for (const auto model : models) {
        int prediction = model->evaluate(trace);
        if (prediction == 0) votes_zero++;
        else if (prediction == 1) votes_one++;
        else throw std::runtime_error("Ensemble::predict: invalid model prediction");
    }
...
std::unique_ptr<refinement_list> EnsembleFactory::random(state_merger* merger) {
    std::cerr << "starting random merging" << std::endl;
    merger->get_eval()->initialize_after_adding_traces(merger);
...
std::unique_ptr<Ensemble> EnsembleFactory::generate(
    ...
    std::cerr << "Starting the creation of ensemble with mode: " << modeToString(mode) << std::endl;
    ...
    for (int i = 0; i < nr_estimators; ++i) {
        ...
        std::cerr << "Created model " << i+1 << "/" << nr_estimators << std::endl;
    ...
    std::cerr << "Ended the creation of ensemble" << std::endl;
    return ensemble;
}
...
std::unique_ptr<Ensemble> EnsembleFactory::load(const std::string &model_path) {
    ...
    while (true) {
        ...
        if (fs::exists(filename) && fs::is_regular_file(filename)) {
            ...
            if (file_stream->is_open()) {
                auto model = Model::from_apta_json(*file_stream);
    ...
    std::cerr << "Loaded ensemble of size" << ensemble->models.size() << "for training set: " << model_path << std::endl;
    return ensemble;
};
